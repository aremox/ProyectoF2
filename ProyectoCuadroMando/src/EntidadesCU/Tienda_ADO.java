...
public class Tienda_ADO extends GeoEntidad {
    ...
    private static final String INSERT = "INSERT INTO TIENDAS "
            + "(id_almacen, calle, numero, cod_postal, poblacion, provincia, telefono, x, y,"
            + "id_tienda) VALUES(?, ?, ?, ?, ?, ?, ?, ?,?,?)";
    private static final String UPDATE = "UPDATE TIENDAS SET id_almacen = ?, calle = ?, numero = ?, cod_postal = ?, poblacion = ?, provincia = ?, telefono = ?,x=?,y=?"
            + " WHERE id_tienda = ?";
